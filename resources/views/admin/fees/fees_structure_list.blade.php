    ...
    <div class="px-3 px-md-5 flex-grow-1 container-p-y">
        <div
            ...
            <div class="d-flex flex-column justify-content-center">
                <h4 class="mb-1 ps-1">Staff List</h4>
            </div>
            <div class="d-flex align-content-center flex-wrap gap-4 mt-2 mt-md-0">
                <a href="{{ route('fees-structure-add-edit', 'add') }}" role="button" class="btn btn-primary">Add Title</a>
            </div>
        </div>
                ...
                <div class="card mb-6">
                    <div class="card-body">
                        <table class="datatables-products table dataTable dtr-column collapsed mt-3" id="myTable"
                            ...
                            <tbody>
                                @if(count($fees_structure) != 0)
                                    @foreach ($fees_structure as $key => $item)
                                        <tr>
                                            <td class="text-center">{{$key + 1}}</td>
                                            <td class="text-center">{{$item->title}}</td> 
                                            <td class="text-center">
                                                <a href="{{ route('fees-structure-add-edit', $item->id) }}" role="button"
                                                    class="btn btn-primary p-0">
                                                    <i class='bxr  bx-edit m-0 p-2' style="font-size: 17px;"></i>
                                                </a>
                                                <a href="{{ route('fees-structure-delete', $item->id) }}" class="btn btn-danger p-0"
                                                    onclick="confirmDelete(event, '{{ route('fees-structure-delete', $item->id) }}')">
