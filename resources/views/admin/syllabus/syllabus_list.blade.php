    ...
    <div class="px-3 px-md-5 flex-grow-1 container-p-y">
        <div
            ...
            <div class="d-flex flex-column justify-content-center">
                <h4 class="mb-1 ps-1">Syllabus List</h4>
            </div>
            <div class="d-flex align-content-center flex-wrap gap-4 mt-2 mt-md-0">
                <a href="{{ route('syllabus-add-edit', 'add') }}" role="button" class="btn btn-primary">Add Syllabus</a>
            </div>
        </div>
                        ...
                        <table class="datatables-products table dataTable dtr-column collapsed mt-3" id="myTable"
                            ...
                            <tbody>
                                @if(count($syllabus_data) != 0)
                                    @foreach ($syllabus_data as $key => $item)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <a href="{{ route('syllabus-add-edit', $item->id) }}" role="button"
                                                    class="btn btn-primary p-0">
                                                    <i class='bxr  bx-edit m-0 p-2' style="font-size: 17px;"></i>
                                                </a>
                                                <a href="{{ route('syllabus-delete', $item->id) }}" class="btn btn-danger p-0"
                                                    onclick="confirmDelete(event, '{{ route('syllabus-delete', $item->id) }}')">
