    ...
    <div class="px-3 px-md-5 flex-grow-1 container-p-y">
        <div
            ...
            <div class="d-flex align-content-center flex-wrap gap-4 mt-2 mt-md-0">
                <a href="{{ route('staff-add-edit', 'add') }}" role="button" class="btn btn-primary">Add Staff</a>
            </div>
        </div>
                                ...
                                @if(count($staff) != 0)
                                    @foreach ($staff as $key => $item)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <a href="{{ route('staff-add-edit', $item->id) }}" role="button"
                                                    class="btn btn-primary p-0">
                                                    <i class='bxr  bx-edit m-0 p-2' style="font-size: 17px;"></i>
                                                </a>
                                                <a href="{{ route('staff-delete', $item->id) }}" class="btn btn-danger p-0"
                                                    onclick="confirmDelete(event, '{{ route('staff-delete', $item->id) }}')">
