    ...
    <div class="px-3 px-md-5 flex-grow-1 container-p-y">
        <div
            ...
            <div class="d-flex flex-column justify-content-center">
                <h4 class="mb-1 ps-1">Gallery List</h4>
            </div>
            <div class="d-flex align-content-center flex-wrap gap-4 mt-2 mt-md-0">
                <a href="{{ route('gallery-add-edit', 'add') }}" role="button" class="btn btn-primary">Add Gallery</a>
            </div>
        </div>
                                ...
                                @if(count($gallery_data) != 0)
                                    @foreach ($gallery_data as $key => $item)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <a href="{{ route('gallery-add-edit', $item->id) }}" role="button"
                                                    class="btn btn-primary p-0">
                                                    <i class='bxr  bx-edit m-0 p-2' style="font-size: 17px;"></i>
                                                </a>
                                                <a href="{{ route('gallery-delete', $item->id) }}" class="btn btn-danger p-0"
                                                    onclick="confirmDelete(event, '{{ route('gallery-delete', $item->id) }}')">
